import { csrfHeaders, ensureCsrfCookie } from "~/lib/auth/csrf";
import type {
  AllauthFlow,
  AllauthSession,
  AuthenticatedUser,
...
/**
 * Browser-side typed client for django-allauth headless.
 *
 ...
 *   2. `X-CSRFToken` on every mutation via csrfHeaders().
 *   3. Tokens / passwords NEVER stored in localStorage or sessionStorage.
 *   4. Exhaustive Result types so callers can't drop MFA / email-verify branches.
 */
