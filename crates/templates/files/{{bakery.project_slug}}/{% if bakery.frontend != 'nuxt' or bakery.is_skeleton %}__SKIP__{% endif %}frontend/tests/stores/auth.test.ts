...
describe("useAuthStore.login", () => {
  ...
  it("returns kind=mfa_required and does NOT store a user on the MFA flow", async () => {
    fetchMock.mockResolvedValueOnce(
      jsonResponse(401, {
  ...
  it("returns kind=invalid_credentials on bare 401", async () => {
    ...
    const result = await useAuthStore().login("a@b.test", "wrong");
...
describe("security invariants", () => {
  it("logout clears the local user", async () => {
    fetchMock.mockResolvedValueOnce(jsonResponse(200, {})); // logout
    const store = useAuthStore();
