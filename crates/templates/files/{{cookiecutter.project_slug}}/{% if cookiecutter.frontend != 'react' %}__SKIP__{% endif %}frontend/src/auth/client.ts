 ...
 *   4. The "Result" return shape is exhaustive — callers can't accidentally
...
export const authClient = {
  /** GET the current session — used to bootstrap auth state on app start. */
  async session(): Promise<AllauthSession> {
    const r = await rawFetch("/auth/session", { method: "GET" });
  ...
  /** Verify the user's TOTP code during the login MFA challenge. */
  ...
  /** Begin TOTP enrollment — returns the otpauth:// URI for QR rendering. */
  async mfaActivateBegin(): Promise<MfaActivateBeginResult> {
    await ensureCsrfCookie();
    const r = await rawFetch("/account/authenticators/totp", { method: "GET" });
    if (r.status === 200) {
  ...
  /** Confirm the TOTP code to complete enrollment. */
  async mfaActivateConfirm(code: string): Promise<{ kind: "ok" } | { kind: "invalid"; status: number }> {
    await ensureCsrfCookie();
  ...
  async recoveryCodes(): Promise<{ codes: string[]; unused: number } | null> {
    await ensureCsrfCookie();
    const r = await rawFetch("/account/authenticators/recovery-codes", { method: "GET" });
  ...
  async passwordResetRequest(email: string): Promise<{ kind: "ok" | "unknown"; status: number }> {
    await ensureCsrfCookie();
    const r = await rawFetch("/auth/password/request", {
      method: "POST",
      headers: csrfHeaders(),
      body: JSON.stringify({ email }),
    });
    return { kind: r.status === 200 ? "ok" : "unknown", status: r.status };
  },
};
