...
definePageMeta({ layout: "account", middleware: "guest" });
...
async function onSubmit(e: Event) {
  ...
  error.value = null;
  fieldErrors.value = {};
  const parsed = schema.safeParse({ email: email.value, password: password.value, confirm: confirm.value });
  if (!parsed.success) {
...
<template>
  <div class="space-y-4">
    ...
    <p class="text-sm text-slate-600 dark:text-slate-400">
      Already have an account? <NuxtLink to="/account/login" class="underline">Sign in</NuxtLink>
    </p>
  </div>
</template>
